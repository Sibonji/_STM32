...
void dyn_display(uint16_t number, int pos_num)
{
    ...
    out = num[pos_num % 4] | position[pos_num % 4];
...
{       
    ...
    dyn_display(newnum, pos_num);
    pos_num++;
           
    button_status = A0_Button_Status(); 
        
    if      (button_status > 0)  newnum   = disp_num + 1;
    else if (button_status == 0) disp_num = newnum;       
}
